...
import joblistdata from './datalayer';
...
const dashboard: any = async (params: any) => {
  try {
    ...
    if(params.searchParams.title || params.searchParams.location || params.searchParams.job_type){
    apiData = apiData.filter((item:any) =>{
      for (var key in params.searchParams) {
        console.log(key)
        // console.log(params.searchParams[key],item[key])
        if (item[key]?.toLowerCase().includes(params.searchParams[key])) return true;
        else return false
      }
    });
    }
